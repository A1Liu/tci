...
fn make_parse_state(grammar: &Grammar) -> TokenStream {
    ...
    for rule in grammar.iter_rules() {
        if rule.cached {
            ...
            cache_fields_def
                .push(quote! { #name: ::hashbrown::HashMap<usize, ::peg::RuleResult<#ret_ty>> });
...
fn compile_rule_export(context: &Context, rule: &Rule) -> TokenStream {
    ...
    quote! {
        #doc
        #visibility fn #name<'input #(, #ty_params)*>(__input: &'input Input #extra_args_def #(, #rule_params)*) -> ::core::result::Result<#ret_ty, ::peg::error::ParseError<PositionRepr>> {
            #![allow(non_snake_case, unused)]
