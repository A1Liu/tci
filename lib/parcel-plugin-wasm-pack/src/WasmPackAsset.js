...
class WasmPackAsset extends Asset {
  ...
  async collectDependencies() {
    ...
    if (path.basename(this.name) === 'Cargo.toml') {
      const src = path.join(path.dirname(this.name), 'src');
      const addDep = (dep) => this.addDependency(rel(this.dir, dep), { includedInParent: true });
      glob(`${src}/**/*.rs`, undefined, (er, files) => files.forEach(addDep));
